...
pi5_dashboard.py
Simple dashboard:
  - Shows live weight from ESP32 (scale/weight, streamed every 500 ms)
...
T_WEIGHT         = "scale/weight"   # ESP32 → realtime weight stream
T_INFO           = "scale/info"     # pill counter → pill count (per-shot)
T_REALTIME       = "scale/realtime" # pill counter → CHT live count
...
state = {"weight": 0.0, "pill_count": 0, "cht_count": 0}
...
def on_connect(client, userdata, flags, reason_code, properties):
    if reason_code.value == 0:
        ...
        client.subscribe(T_WEIGHT)
        client.subscribe(T_INFO)
        client.subscribe(T_REALTIME)
    else:
...
def on_message(client, userdata, msg):
    ...
    elif msg.topic == T_REALTIME:
        cht_count = int(data.get("cht_count", 0))
        with state_lock:
            state["cht_count"] = cht_count
        socketio.emit("update", {"cht_count": cht_count})

_mqtt_client = None

def start_mqtt():
    global _mqtt_client
    _mqtt_client = mqtt.Client(mqtt.CallbackAPIVersion.VERSION2, client_id="Pi5_Dashboard")
    _mqtt_client.on_connect    = on_connect
    _mqtt_client.on_disconnect = on_disconnect
    _mqtt_client.on_message    = on_message
    _mqtt_client.reconnect_delay_set(1, 30)
...
<body>
  ...
  <div class="row">
    ...
    <div class="card">
      ...
      <div class="value" id="cht_count">{{ cht_count }}</div>
    </div>
  </div>
  ...
  <script>
    ...
    socket.on("update", d => {
      ...
      if (d.cht_count !== undefined) document.getElementById("cht_count").textContent = d.cht_count;
    });
...
def api_save():
    ...
    try:
        ...
        if _mqtt_client and _mqtt_client.is_connected():
            _mqtt_client.publish("scale/command", "finish")
            log.info("MQTT tx [scale/command]: finish")
...
def _sheet_init():
    try:
        ...
        log.error("Sheet init failed: %s", exc)

if __name__ == "__main__":
    threading.Thread(target=_sheet_init, daemon=True).start()
